local MixtureTableV2, parent = torch.class('nn.MixtureTableV2', 'nn.Module')

function MixtureTableV2:__init(dim)
   parent.__init(self)
   self.dim = dim
...
function MixtureTableV2:updateOutput(input) 
...
--      if self.batchSize ~= batchSize or then
...
function MixtureTableV2:updateGradInput(input, gradOutput)
   local gaterInput, expertInputs = table.unpack(input)
...
function MixtureTableV2:type(type, tensorCache)
   self._gaterView = nil
   self._expert = nil
...
function MixtureTableV2:clearState()
   nn.utils.clear(self, {
     '_gaterView',
     '_expert',
     '_expertView',
