...
function SLSTMModel:buildNet()
    ...
    local decoder_step = nn.Sequential():add(
            nn.LookupTableMaskZero(self.vocabSize, self.dimSize)
        )
    for l=1,self.layerSize do
...
function SLSTMModel:encoderForward(encoderInput)
    self:forget()
...
function SLSTMModel:encoderBackward(encoderInput)
    self.encoderGradient = self.encoderGradient or torch.Tensor()
    self.encoderGradient = self.encoderGradient:typeAs(self.encoder.output)
    local encoderGradient = self.encoderGradient:resizeAs(
        self.encoder.output):zero()

    self.encoder:backward(encoderInput, encoderGradient)

end

function SLSTMModel:decoderForward(decoderInput)
    maxSteps = decoderInput:size(2)
    self.logitsOutput = {}
    local Y = decoderInput:t()
    for step=1,maxSteps do
        local Yt = Y[step]
        self.logitsOutput[step] = self.decoder:forward(Yt)
    end
    return self.logitsOutput
end

function SLSTMModel:decoderBackward(decoderInput, gradOutput)
    local maxSteps
    local Y
    ...
    for step=maxSteps,1,-1 do
        self.decoder:backward(Y[step], gradOutput[step])
    end
    self:backwardConnect()
end

function SLSTMModel:lossForward(logits, output)
...
function SLSTMModel:lossBackward(logits, output)
    local stepSize = #logits
...
function SLSTMModel:trainStep(encoderInput, decoderInput, decoderOutput)
    ...
    local function feval(params)
        ...
        self:decoderBackward(decoderInput, gradOutput)
        self:encoderBackward(encoderInput)
...
function SLSTMModel:greedyDecode(encoderInput, outputScore, useOutputBuffer,
                                 computeLogSoftMax)
--[[ 
